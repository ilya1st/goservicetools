...
func (app *helloApp) handleHello(conn net.Conn) {
	defer conn.Close()
...
func (app *helloApp) SystemStart(graceful bool) error {
	...
	app.rMutex.RUnlock()
	if l == nil {
		goservicetools.GetSystemLogger().Fatal().Msg("No listener prepared for hello service")
	}
	go func() {
		for {
			app.rMutex.RLock()
			...
			c, err := l.Accept()
			if err != nil {
				goservicetools.GetSystemLogger().Info().Msgf("Hello listener stopped accept connections: %v", err)
			}
			go app.handleHello(c)
		}
	}()
	return nil
}
